...
| M1.6 | Arrival and withdrawal are the walk-in and farewell segments of one generated clip of the figure (assets/anim, 12 fps): he enters mid-stride from the right edge of the companion window and stops on the resting spot in at most 2.2 seconds; on withdrawal he raises a palm, turns, and walks out to the right in at most 3.2 seconds. His feet move the true scene distance (no sliding), verified by `test/sprite-player.test.js` placement checks. The window is placed once, on the display that holds the saved position, and does not move while present. He never follows the cursor. During arrival, at most 5 percent of frames exceed 20 ms (Electron performance panel, environment above). | screen recording of one full darshan plus the performance panel capture; `test/sprite-player.test.js` for placement and timing; `src/darshan.js` ARRIVAL_MS 2200 and WITHDRAWAL_MS 3200 are upper bounds on the 2083 ms walk-in and 3167 ms farewell | proposed |
...
|----|------------------|-------|--------|
| UI1 | The figure is a flipbook of frames cut from one generated video of a single Krishna painting (`assets/anim/manifest.js` + four WebP sheets, 3.0 MB): walk-in 12 fps, idle 8 fps ping-pong loop with breathing and blinks, teaching gesture 10 fps on expand, farewell 12 fps. Durations come from the manifest and `src/darshan.js`. Without the manifest the renderer falls back to the single still and the CSS slide. | `test/sprite-player.test.js`, `test/micro-motion.test.js` | held |
