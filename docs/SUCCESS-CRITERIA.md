...
|----|------------------|-------|--------|
| UI1 | The figure is a flipbook of frames cut from one generated video per style (`assets/anim/<style>/`, four WebP sheets each, assembled into `assets/anim/manifest.js`): six styles ship, realistic (default), cartoon, painterly, gyan, warrior, and pixel (the warrior pixelated by `scripts/anim/pixelate-style.py`, drawn with image smoothing off), about 18 MB in total; the style is a setting (tray Figure menu, `krshna style <name>`) validated against one list in `src/config.js`, and a present darshan switches on the spot: walk-in 12 fps, idle 8 fps ping-pong loop with breathing and blinks, teaching gesture 10 fps on expand, farewell 12 fps. Durations come from the manifest and `src/darshan.js`. Without the manifest the renderer falls back to the single still and the CSS slide. | `test/sprite-player.test.js`, `test/renderer-sprite.test.js`, `test/micro-motion.test.js` | held |
