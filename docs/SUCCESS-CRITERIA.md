...
|----|------------------|-------|--------|
| UI1 | Articulated arrival reveals a message after 3.2 seconds; farewell and walking withdrawal hide the native window after 3.2 seconds. Reduced motion reveals without delay or an animation loop. Reading has restrained breathing and blinking. Three selectable art styles share identical choreography. | `test/character-motion.test.js`, `test/darshan.test.js`, `test/renderer.test.js`; visual browser preview | manual (native motion) |
