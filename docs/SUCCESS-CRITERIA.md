...
|----|------------------|-------|--------|
| UI1 | One `assets/krishna.png` figure glides in from the right and withdraws the same way, both durations driven by `src/darshan.js` (arrival glide and message reveal at 1.1 seconds; withdrawal glide and native window hide at 0.9 seconds). The present figure breathes on a 4-second ease-in-out scale from 1.000 to 1.012 and back. Reduced motion is fixed, and absence has no animation selector or JavaScript loop. Native window placement and the CPU sample are not yet accepted, so this stays proposed until those manual checks pass. | `test/micro-motion.test.js`, `test/darshan.test.js`, `test/renderer.test.js`; visual browser review | proposed |
...
| UI5 | Micro-motion: breathing scale 1.000 to 1.012 with a 4 s ease-in-out period while present; one settle of 6 px over 400 ms on arrival; blink omitted; every loop stops when absent or reduced-motion. | test/micro-motion.test.js | held |
