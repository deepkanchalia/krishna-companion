...
|----|------------------|-------|--------|
| UI1 | The figure is a flipbook of frames cut from one generated video of a single Krishna painting (`assets/anim/manifest.js` + four WebP sheets, 3.0 MB): walk-in 12 fps, idle 8 fps ping-pong loop with breathing and blinks, teaching gesture 10 fps on expand, farewell 12 fps. Durations come from the manifest and `src/darshan.js`. Without the manifest the renderer falls back to the single still and the CSS slide. | `test/sprite-player.test.js`, `test/renderer-sprite.test.js`, `test/micro-motion.test.js` | held |
